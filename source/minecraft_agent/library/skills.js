...
export async function digDown(bot, distance = 10) {
    /**
     * Digs a hole straight down from the current position.
     * @param {MinecraftBot} bot, reference to the minecraft bot.
     * @param {number} distance, the distance to dig down. Defaults to 10.
     * @returns {Promise<boolean>} true if the hole was dug, false otherwise.
     * @example
     * await skills.digDown(bot, 5);
     **/
    let position = bot.entity.position;
    for (let i = 0; i < distance; i++) {
        if (bot.interrupt_code) return false;
        let block = bot.blockAt(position.offset(0, -i, 0));
        if (!block || block.name === 'air') continue;
        if (block.name === 'water' || block.name === 'lava') {
            log(bot, `Stopped digging down due to liquid at depth ${i}.`);
            break;
        }
        if (!bot.canDigBlock(block)) {
            log(bot, `Cannot dig block ${block.name} at depth ${i}.`);
            continue;
        }
        await bot.dig(block);
        log(bot, `Dug ${block.name} at depth ${i + 1}.`);
    }
    return true;
}

export async function buildSimpleHut(bot, size = 3) {
    /**
     * Builds a simple wooden hut at the current position.
     * @param {MinecraftBot} bot, reference to the minecraft bot.
     * @param {number} size, the size of the hut (radius). Defaults to 3.
     * @returns {Promise<boolean>} true if the hut was built, false otherwise.
     * @example
     * await skills.buildSimpleHut(bot, 4);
     **/
    if (bot.buildManager) {
        try {
            const result = await bot.buildManager.handleBuildCommand('hut', [size.toString()]);
            if (result.success) {
                log(bot, `Started building a hut with size ${size}.`);
                return true;
            } else {
                log(bot, `Failed to start building hut: ${result.reason || result.error}`);
                return false;
            }
        } catch (error) {
            log(bot, `Error building hut: ${error.message}`);
            return false;
        }
    } else {
        log(bot, 'Build manager not available.');
        return false;
    }
}

export async function buildWall(bot, length = 10, height = 3) {
    /**
     * Builds a simple wall from the current position.
     * @param {MinecraftBot} bot, reference to the minecraft bot.
     * @param {number} length, the length of the wall. Defaults to 10.
     * @param {number} height, the height of the wall. Defaults to 3.
     * @returns {Promise<boolean>} true if the wall was built, false otherwise.
     * @example
     * await skills.buildWall(bot, 15, 4);
     **/
    if (bot.buildManager) {
        try {
            const result = await bot.buildManager.handleBuildCommand('wall', [length.toString(), height.toString()]);
            if (result.success) {
                log(bot, `Started building a wall with length ${length} and height ${height}.`);
                return true;
            } else {
                log(bot, `Failed to start building wall: ${result.reason || result.error}`);
                return false;
            }
        } catch (error) {
            log(bot, `Error building wall: ${error.message}`);
            return false;
        }
    } else {
        log(bot, 'Build manager not available.');
...
export async function getBuildStatus(bot) {
    /**
     * Gets the current building status.
     * @param {MinecraftBot} bot, reference to the minecraft bot.
     * @returns {Promise<object>} the build status object.
     * @example
     * let status = await skills.getBuildStatus(bot);
     **/
    if (bot.buildManager) {
        const status = bot.buildManager.getBuildStatus();
        if (status.building) {
            log(bot, `Currently building ${status.session.buildingType} - ${status.session.progress}% complete (Phase: ${status.session.phase})`);
        } else {
            log(bot, 'No active construction.');
        }
        return status;
    } else {
        log(bot, 'Build manager not available.');
...
export async function stopBuilding(bot) {
    /**
     * Stops the current building task.
     * @param {MinecraftBot} bot, reference to the minecraft bot.
     * @returns {Promise<boolean>} true if building was stopped, false otherwise.
     * @example
     * await skills.stopBuilding(bot);
     **/
    if (bot.buildManager) {
        try {
            const result = await bot.buildManager.stopBuild('User requested via skill');
            if (result.success) {
                log(bot, 'Building stopped successfully.');
                return true;
            } else {
                log(bot, `Failed to stop building: ${result.reason}`);
                return false;
            }
        } catch (error) {
            log(bot, `Error stopping building: ${error.message}`);
            return false;
        }
    } else {
        log(bot, 'Build manager not available.');
...
export async function clearArea(bot, x1, y1, z1, x2, y2, z2) {
    /**
     * Clears all blocks in the specified area.
     * @param {MinecraftBot} bot, reference to the minecraft bot.
     * @param {number} x1, y1, z1, coordinates of first corner.
     * @param {number} x2, y2, z2, coordinates of second corner.
     * @returns {Promise<boolean>} true if area was cleared, false otherwise.
     * @example
     * await skills.clearArea(bot, 10, 60, 10, 15, 65, 15);
     **/
                ...
                if (block && block.name !== 'air') {
                    if (bot.canDigBlock(block)) {
                        try {
                            ...
                            log(bot, `Failed to dig block at (${x}, ${y}, ${z}): ${error.message}`);
                        }
                    }
                }
            }
    ...
    log(bot, `Cleared ${blocksCleared} blocks in the specified area.`);
    return true;
}

export async function buildFoundation(bot, x1, y, z1, x2, z2, materialType = 'oak_planks') {
    /**
     * Builds a foundation (flat floor) in the specified area.
     * @param {MinecraftBot} bot, reference to the minecraft bot.
     * @param {number} x1, z1, coordinates of first corner.
     * @param {number} y, the Y level for the foundation.
     * @param {number} x2, z2, coordinates of second corner.
     * @param {string} materialType, the material to use. Defaults to 'oak_planks'.
     * @returns {Promise<boolean>} true if foundation was built, false otherwise.
     * @example
     * await skills.buildFoundation(bot, 10, 60, 10, 15, 15, 'stone');
     **/
    const minX = Math.min(x1, x2);
    const maxX = Math.max(x1, x2);
    const minZ = Math.min(z1, z2);
    ...
    const requiredBlocks = (maxX - minX + 1) * (maxZ - minZ + 1);
    const inventory = world.getInventoryCounts(bot);
    
    if (!inventory[materialType] || inventory[materialType] < requiredBlocks) {
        log(bot, `Not enough ${materialType}. Need ${requiredBlocks}, have ${inventory[materialType] || 0}.`);
        return false;
    }
    ...
    for (let x = minX; x <= maxX; x++) {
        for (let z = minZ; z <= maxZ; z++) {
            ...
            try {
                const success = await placeBlock(bot, materialType, x, y, z);
                if (success) {
                    blocksPlaced++;
                }
            } catch (error) {
                log(bot, `Failed to place block at (${x}, ${y}, ${z}): ${error.message}`);
            }
        }
    }
    
    log(bot, `Built foundation with ${blocksPlaced} blocks.`);
